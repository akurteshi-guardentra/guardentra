...
### Sprint 4 — Multi-user orgs & auth hygiene (NEW — addresses §2 items 5-7)
- Teammate invite flow: invite by email → join an existing org as a non-admin member, instead of every signup creating a new org
- Make `admin` vs member meaningful: enforce role server-side (not just in Firestore delete rules), reflect it in the UI
